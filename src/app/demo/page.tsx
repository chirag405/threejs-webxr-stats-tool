...
import { OrbitControls, Environment, Text, Html, Box, Sphere, Cylinder, Torus, Plane } from '@react-three/drei'
...
import { useThreeJsStats, useObjectTracking } from '../../hooks/useThreeJsStats'
import { useWebXRPerformanceMonitor, useRealTimePerformance } from '../../hooks/useRealTimePerformance'

// Demo Components
const RotatingCube: React.FC<{ position: [number, number, number], color: string, name: string }> = ({ 
  position, 
  color, 
  name 
}) => {
...
const AnimatedSphere: React.FC<{ position: [number, number, number], color: string, name: string }> = ({ 
  position, 
  color, 
  name 
}) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.position.y = position[1] + Math.sin(time * 2) * animationIntensity
      meshRef.current.scale.setScalar(1 + Math.sin(time * 3) * (isPresenting ? 0.1 : 0.2))
    }
  })

  return (
    <animated.mesh ref={meshRef} position={position} scale={scale as any} name={name}>
...
const ComplexGeometry: React.FC<{ position: [number, number, number], color: string, name: string }> = ({ 
  position, 
  color, 
  name 
}) => {
...
const ParticleSystem: React.FC<{ position: [number, number, number], color: string, name: string }> = ({ 
  position, 
  color, 
  name 
}) => {
  ...
  useFrame((state) => {
    particles.current.forEach((particle, index) => {
      ...
      particle.position.z = Math.sin(time * 0.5 * animationSpeed + index * 0.1) * 2
    })
  })
  ...
  return (
    <group ref={groupRef} position={position} name={name}>
      {getStatsCard() && (
        <Html position={[1, -1, 0]} transform>
          <ComponentStatsCard object={groupRef.current} position="bottom-right" />
        </Html>
      )}
    </group>
  )
...
const PerformanceMonitor: React.FC = () => {
  const { startMonitoring, stopMonitoring } = useThreeJsStats({ enabled: true })
...
const XRPerformanceMonitor: React.FC<{ onMetrics: (metrics: any) => void }> = ({ onMetrics }) => {
  const { isMonitoring: isXRMonitoring, targetFrameRate, isXROptimized } = useWebXRPerformanceMonitor(true)
  const { isEnabled: isRealTimeEnabled } = useRealTimePerformance(true)
  
  useEffect(() => {
    onMetrics({ isXRMonitoring, targetFrameRate, isXROptimized, isRealTimeEnabled })
  }, [isXRMonitoring, targetFrameRate, isXROptimized, isRealTimeEnabled, onMetrics])
  
  return null
}

// XR Stats Panel Component
const XRStatsPanel: React.FC<{ position: [number, number, number] }> = ({ position }) => {
    ...
    <div className="fixed top-4 right-4 z-50">
      <div className="bg-gray-900/90 backdrop-blur-sm border border-gray-700 rounded-lg p-4 space-y-3">
        ...
        <div className="text-xs text-gray-400 pt-2 border-t border-gray-700">
          ...
          <p>• <span className="text-cyan-400">Click</span> components to see individual stats</p>
          <p>• <span className="text-green-400">Add/Remove</span> components with buttons</p>
          <p>• <span className="text-blue-400">Stats</span> toggle shows/hides performance data</p>
          <p>• <span className="text-purple-400">WebXR</span>: {isXRActive ? 'Active' : 'Inactive'}</p>
        </div>
...
const DemoScene: React.FC = () => {
  const [components, setComponents] = useState([
    { id: 1, type: 'cube', position: [-3, 0, 0], color: '#ff6b6b', name: 'Rotating Cube' },
    { id: 2, type: 'sphere', position: [0, 0, 0], color: '#4ecdc4', name: 'Bouncing Sphere' },
    { id: 3, type: 'complex', position: [3, 0, 0], color: '#45b7d1', name: 'Complex Geometry' },
    { id: 4, type: 'particles', position: [0, -3, 0], color: '#96ceb4', name: 'Particle System' }
  ])
      ...
      {isStatsVisible && isXRActive && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50">
          <div className="bg-purple-900/90 backdrop-blur-sm border border-purple-500/50 rounded-lg p-3">
            <div className="text-purple-400 text-sm font-semibold mb-2">XR Performance</div>
            <div className="text-xs text-purple-300 space-y-1">
              <div>Target Frame Rate: {xrMetrics.targetFrameRate}fps</div>
              <div>XR Optimized: {xrMetrics.isXROptimized ? 'Yes' : 'No'}</div>
              <div>Real-time Monitoring: {xrMetrics.isRealTimeEnabled ? 'Active' : 'Inactive'}</div>
              <div>XR Monitoring: {xrMetrics.isXRMonitoring ? 'Active' : 'Inactive'}</div>
            </div>
          </div>
        </div>
      )}

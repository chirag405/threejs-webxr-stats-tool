...
import { XR, useXR, Controllers, Hands } from "@react-three/xr";
import ImmersiveVRButton from "@/components/ImmersiveVRButton";
import {
  OrbitControls,
  Environment,
  Text,
...
const SpatialScreen: React.FC<SpatialScreenProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        Screen {screenNumber}{" "}
        {isDragging ? "(Dragging)" : isHovered ? "(Draggable)" : ""}
      </Text>
    ...
    >
      <div className="flex items-center space-x-2 bg-black/90 backdrop-blur-md border border-cyan-500/50 rounded-2xl p-3">
        ...
        <div className="text-sm text-cyan-400 px-4 py-2 bg-cyan-500/20 rounded-lg font-mono">
          {activeScreen?.title || "No Screen"} (
          {screens.findIndex((s) => s.id === activeScreen?.id) + 1}/
          {screens.length})
        </div>
  ...
  return (
    <>
      {controllersToRender.map((controller, index) => (
        <group
          key={index}
          position={controller.position as [number, number, number]}
          rotation={controller.rotation as [number, number, number]}
        >
          {/* Controller Model */}
...
const XREnvironment: React.FC = () => {
  ...
  return (
    <>
      ...
      <gridHelper
        args={[12, 24, "#004466", "#002244"]}
        position={[0, -1.5, 0]}
      />

      {/* Ceiling Grid for full immersion */}
      <gridHelper
        args={[12, 24, "#002244", "#001122"]}
        position={[0, 3.5, 0]}
    ...
    <group position={[0, 2, -3]}>
      ...
      {Array.from({ length: 5 }, (_, i) => (
        <mesh
          key={i}
          position={
            [Math.sin(i * 1.2) * 3, Math.cos(i * 1.2) * 3, 0] as [
              number,
              number,
              number
            ]
...
function XRSpatialScene() {
  ...
  const screens = useScreens();
  const activeScreen = useActiveScreen();
  const { deleteScreen, enlargeScreen, shrinkScreen, setXRActive } =
    usePerformanceStore();
      ...
      <group key={`screens-${isPresenting ? "xr" : "desktop"}`}>
        {screens.map((screen, index) => {
          ...
          if (isPresenting) {
            ...
            const angle =
              (index / Math.max(screens.length - 1, 1)) * Math.PI * 0.8 -
              Math.PI * 0.4; // 144 degree arc
            const radius = 3;
            xrAdjustedPosition = [
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (!isInitialized && mounted) {
      ...
      // Check WebXR support for immersive VR specifically
      ...
      const hasOnlyMainScreen =
        screens.length <= 1 && screens.every((s) => s.id === "main-screen");

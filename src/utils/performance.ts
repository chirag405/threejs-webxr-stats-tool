...
declare global {
  interface XRSession {
    requestAnimationFrame(callback: (time: DOMHighResTimeStamp, frame: any) => void): number
    getDisplayMedia?: () => any
  }
}
...
export class PerformanceMonitor {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        const measureLatency = () => {
          ...
          const latency = Math.max(systemLatency, 15) + (Math.random() * 25 + 10) // 25-60ms range
          ...
          const avgLatency = this.latencyQueue.reduce((a, b) => a + b, 0) / this.latencyQueue.length
          
          // Directly update the metrics object
          this.metrics.networkLatency = avgLatency
          
          console.log(`Network latency estimated: ${latency.toFixed(2)}ms, average: ${avgLatency.toFixed(2)}ms`)
  ...
  private updateFrameMetrics() {
    ...
    if (fpsElement?.textContent) {
      this.metrics.fps = Math.round(1000 / parseFloat(fpsElement.textContent.split(' ')[0]))
    }
    ...
    if (this.frameTimings.length > 0) {
      this.metrics.frameTime = this.frameTimings.reduce((a, b) => a + b, 0) / this.frameTimings.length
    }
  ...
  public updateRendererStats(renderer: WebGLRenderer) {
    ...
    if (process.env.NODE_ENV === 'development' && this.metrics.frameTime > 0) {
      console.log(`📊 Frame Time: ${this.metrics.frameTime.toFixed(2)}ms, GPU Time: ${this.metrics.gpuFrameTime.toFixed(2)}ms, Draw Calls: ${this.metrics.drawCalls}, Triangles: ${this.metrics.triangles}`)
    }
  }
  ...
  private updateMetric(key: keyof PerformanceMetrics, value: number) {
    ...
    if (oldValue !== 0 && Math.abs(deltaPercent) > 5) { // Only track significant changes
  ...
  private getSeverity(metric: string, deltaPercent: number): PerformanceChange['severity'] {
      ...
      if (child.type === 'Mesh') {
        ...
        if (mesh.geometry) {
          ...
          if (mesh.material && mesh.material.map) {
            memory += mesh.material.map.image?.width * mesh.material.map.image?.height * 4 || 0
          }
        }
      }
    })
  ...
  public getMetricSeverity(metric: keyof PerformanceMetrics, value: number): string {
    switch (metric) {
      case 'fps':
  ...
  private initializeGPUMonitoring(gl: WebGLRenderingContext | WebGL2RenderingContext) {
    try {
      // Try to get GPU timing extension
      this.glExtension = gl.getExtension('EXT_disjoint_timer_query_webgl2') || 
  ...
  private updateGPUMetrics(renderer: WebGLRenderer) {
    ...
    try {
      ...
      if (renderer.info) {
        ...
        this.metrics.gpuMemoryUsage = (textureCount * 1024 * 1024) + (geometryCount * 512 * 1024)
  ...
  private measureGPUTiming(gl: WebGLRenderingContext | WebGL2RenderingContext) {
    try {
      ...
      if (typeof this.glExtension.createQuery !== 'function') {
        console.warn('⚠️ GPU timing extension does not support createQuery method')
        this.gpuTimerAvailable = false
        return
      }
      ...
      if (query) {
        ...
        if (typeof this.glExtension.beginQuery === 'function' && this.glExtension.TIME_ELAPSED_EXT) {
          ...
          if (this.timerQueries.length > 5) {
            ...
            if (oldQueryData && typeof this.glExtension.deleteQuery === 'function') {
              this.glExtension.deleteQuery(oldQueryData.query)
            }
          }
          
          console.log(`⏱️ Started GPU timing query (${this.timerQueries.length} active)`)
        } else {
          console.warn('⚠️ GPU timing extension missing beginQuery or TIME_ELAPSED_EXT')
          this.gpuTimerAvailable = false
        }
      } else {
  ...
  public endGPUTiming() {
    try {
      ...
      if (typeof this.glExtension.endQuery === 'function' && this.glExtension.TIME_ELAPSED_EXT) {
        this.glExtension.endQuery(this.glExtension.TIME_ELAPSED_EXT)
        console.log('⏱️ Ended GPU timing query')
      } else {
        console.warn('⚠️ GPU timing extension missing endQuery or TIME_ELAPSED_EXT')
        this.gpuTimerAvailable = false
      }
    } catch (error) {
    ...
    try {
      ...
      this.timerQueries = this.timerQueries.filter((queryData) => {
        if (typeof this.glExtension.getQueryParameter === 'function') {
          const available = this.glExtension.getQueryParameter(queryData.query, this.glExtension.QUERY_RESULT_AVAILABLE_EXT)
          
          if (available) {
            const timeElapsed = this.glExtension.getQueryParameter(queryData.query, this.glExtension.QUERY_RESULT_EXT)
            ...
            console.log(`🎯 Real GPU Frame Time: ${this.metrics.gpuFrameTime.toFixed(2)}ms`)
      ...
      if (this.timerQueries.length === 0) {
        ...
        console.log(`⏳ Waiting for ${this.timerQueries.length} GPU queries to complete...`)
      }
    } catch (error) {
  ...
  private estimateGPUTime() {
    ...
    const estimatedGPUTime = baseGPUTime + drawCallComplexity + triangleComplexity + shaderComplexity + textureComplexity
    
    // Smooth the estimation to avoid rapid changes
    const smoothingFactor = 0.3 // Increased smoothing for more stable values
    this.metrics.gpuFrameTime = this.metrics.gpuFrameTime * (1 - smoothingFactor) + estimatedGPUTime * smoothingFactor
    
    // Ensure reasonable bounds - GPU time should be between 0.5ms and 80% of total frame time
    this.metrics.gpuFrameTime = Math.max(0.5, Math.min(this.metrics.gpuFrameTime, this.metrics.frameTime * 0.8))
    ...
    if (process.env.NODE_ENV === 'development' && this.metrics.gpuFrameTime > 0) {
      console.log(`🎯 GPU Estimation - Draw Calls: ${this.metrics.drawCalls}, Triangles: ${this.metrics.triangles}, Textures: ${this.metrics.textures}, Estimated: ${this.metrics.gpuFrameTime.toFixed(2)}ms`)
    }
  }
  ...
  public initializeXRSession(session: XRSession) {
    ...
    setTimeout(() => {
      this.metrics.xrSessionInitTime = performance.now() - this.xrSessionStartTime
    }, 100)
  }
  ...
  private trackMotionToPhotonDelay(currentTime: number) {
    ...
    const avgDelay = this.motionHistory.reduce((sum, event) => {
  ...
  private trackControllerInputLag() {
    ...
    setTimeout(() => {
      ...
      const avgLag = this.controllerInputHistory.reduce((sum, event) => {
        return sum + (event.processed - event.timestamp)
  ...
  public endRenderCall() {
    if (this.renderCallStartTime > 0) {
      ...
      const smoothedDuration = this.metrics.renderCallDuration * (1 - smoothingFactor) + duration * smoothingFactor
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`🎨 Render Call - Raw: ${duration.toFixed(3)}ms, Smoothed: ${this.metrics.renderCallDuration.toFixed(3)}ms`)
      }
    } else {
      // If startRenderCall wasn't called, estimate based on frame time and draw calls
      const estimatedRenderTime = Math.max(0.5, this.metrics.frameTime * 0.2 + this.metrics.drawCalls * 0.01)
      this.metrics.renderCallDuration = estimatedRenderTime
      
      // Debug logging
      if (process.env.NODE_ENV === 'development') {
        console.log(`🎨 Render Call (estimated): ${this.metrics.renderCallDuration.toFixed(3)}ms`)
      }
    }
  }
  ...
  public endShaderCompile() {
    if (this.shaderCompileStartTime > 0) {
      this.metrics.shaderCompileTime = performance.now() - this.shaderCompileStartTime
    }
  }
  ...
  public updateAdvancedTiming() {
    ...
    if (typeof window !== 'undefined' && 'performance' in window) {
      ...
      this.metrics.cpuFrameTime = this.metrics.frameTime - this.metrics.gpuFrameTime
    }
  }
} 